...
my $widgit_1_combo = "$widgit_1_lable . $widgit_1_label_p2";
...
our $defcon = "cnf.srs";
my $fname;
my @ct_var = qw/EMAIL PHONE VOICEMAIL STORE TECHBENCH/;
my @cur_config;
...
$mw->title('Customer Care Log - Version 1.0');
...
my $lb = $mw -> Listbox(-selectmode => "single", -width=>20, -height=>5) ->pack(-padx => '10', -pady => '10',-anchor => 'nw');
...
our $cur_date = sprintf("%02d/%02d/%02d", $mon + 1, $mday, $year % 100,);
our $cur_time = sprintf("%02d:%02d", $hour, $min,);
}
...
&update_conf;
$label_fill_1->configure (-text => "$cur_wk_file");
open ( $outfile, $cur_wk_file) or die $!;
my @file_temp = <$outfile>;
close($outfile);
foreach (@file_temp) { $text_area1 -> insert('end',$_);}
&f_open;
...
#Save changes to document in current side view
sub save_change {
$answer2 = $mw->Dialog(-title => 'Please Reply', 
